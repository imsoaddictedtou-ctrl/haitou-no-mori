...
from scraper import (
    ...
    check_irbank_status, check_kabutan_status,
)
...
@st.cache_data(ttl=600)
def _cached_kabutan_blocked() -> bool:
    """株探のブロック有無を10分キャッシュ。IRBankがブロック中の時だけ呼び出す想定。"""
    return check_kabutan_status()
...
def main():
    ...
    if _cached_irbank_blocked():
        if _cached_kabutan_blocked():
            st.error(
                "⚠️ 現在IRバンク・株探ともに混み合っており、データが「不明」や「0円」で"
                "表示されることがあります。みんかぶ経由で一部データを補完していますが、"
                "スクリーニングや配当カレンダーの結果が少なく見える場合は、時間をおいて再度お試しください。"
            )
        else:
            st.warning(
